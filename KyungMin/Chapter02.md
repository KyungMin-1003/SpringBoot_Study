...
기능 (API Name),Method,Endpoint,Headers,Request Body / Parameters
내 정보 조회,GET,/user/me,Authorization: bearer {Token},없음
내 포인트 조회,GET,/user/me/points,Authorization: bearer {Token},없음
회원가입,POST,/auth/singup,Content-Type: application/json,"Body: loginId, password, name, phoneNumber"
로그인,POST,/auth/login,Content-Type: application/json,"Body: email, password"
탈퇴,DELETE,/user/me,Content-Type: application/json,Body: password
